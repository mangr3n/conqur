...
getBus = function (name) {
    ...
    if (Registry.lookup(_registryName) == null) {
            ...
            castHandlers: {
                init: function (self, state, msg) {
                    ...
                    for (var idx = 0; idx < handlers.length; idx++) {
                        ...
                        var handlerId = call(self(), { type: 'registerHandler', options: options, event: event_1, handler: handler });
                    }
                    return state;
                },
                        ...
                        while (state._queueInProcess.length > 0) {
                            ...
                            try {
                                ...
                                if (result === undefined || result) {
                                    if (options.once) {
                                        call(self(), { type: 'removeHandler', id: id });
                                    }
                                }
                                else {
                                ...
                                call(self(), { type: 'sendEvent', event: { type: 'error', error: error, source: handleEntry } });
                            }
                        }
        ...
        Registry.create(bus_1, _registryName);
    }
    return {
        getBus: getBus,
        initialize: function (handlers) {
            cast(Registry.lookup(_registryName), { type: 'init', handlers: handlers });
        },
        sendEvent: function (event) {
            call(Registry.lookup(_registryName), { type: 'sendEvent', event: event });
            return;
        },
        handle: function (event, handler, options) {
            if (options === void 0) { options = {}; }
            return call(Registry.lookup(_registryName), { type: 'registerHandler', event: event, handler: handler, options: options });
        },
        handleOnce: function (event, handler) {
            return call(Registry.lookup(_registryName), { type: 'registerHandler', event: event, handler: handler, options: { once: true } });
        },
        ...
        debug: function (turnOn) {
            ...
            return call(Registry.lookup(_registryName), { type: 'debug', turnOn: turnOn });
        },
        logEvents: function (turnOn) {
            if (turnOn === void 0) { turnOn = true; }
            return call(Registry.lookup(_registryName), { type: 'logEvents', turnOn: turnOn });
        }
    };
};
export var Bus = getBus('mainBus');
